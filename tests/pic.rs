...
fn annex_i_qr_as_pic() {
    ...
    let expected = include_str!("data/annex_i_qr_as_pic.pic");
...
fn annex_i_micro_qr_as_pic() {
    ...
    let expected = include_str!("data/annex_i_micro_qr_as_pic.pic");
    assert_eq!(&image, expected);
}
...
fn rmqr_as_pic() {
    ...
    .unwrap();
    let image = code.render::<Color>().build();
    let expected = include_str!("data/rmqr_as_pic.pic");
    assert_eq!(&image, expected);
}
